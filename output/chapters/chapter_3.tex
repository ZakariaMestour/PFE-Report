...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{images/ch3_core_data_model.png}
...
\texttt{ScfProgramCashflow} represents an invoice or cashflow entry inside a program. It stores invoice number, supplier, buyer, amount, currency, invoice date, due date, payment terms, validation status, validation errors, and validation warnings.
...
\texttt{ProgramSnapshotService} supports copying an existing program. It duplicates the configuration and its sub-entities, including anchors, counterparties, and fee catalogues, while excluding cashflows. The copied program starts in draft status.

That exclusion is sensible. Cashflows represent operational entries linked to a particular program life. Copying them into a new draft could create false business history. The feature therefore copies reusable configuration, not transactional evidence.

\section{Invoice Upload and Cashflow Management}
\label{sec:invoice_cashflow}

\subsection{Cashflow Validation Rules}
\label{subsec:cashflow_validation}

Cashflows represent invoice-level data inside a program. The \texttt{CashflowUpdateController} accepts invoice details through \texttt{ProgramCashflowRequest}. The service validates duplicate invoice numbers, date consistency, and positive amounts. The result is stored as a \texttt{ScfProgramCashflow} with validation status, errors, and warnings.

\begin{algorithm}[H]
    \DontPrintSemicolon
    \Data{Program identifier and submitted invoice data}
    \Result{Persisted cashflow with validation status}
    \BlankLine
    Load the target program configuration\;
    Check whether the invoice number already exists for the program\;
    Verify that invoice date is not later than due date\;
    Verify that invoice amount is positive\;
    \If{blocking validation errors exist}{
        Store the entry with error status and validation details\;
    }
    \Else{
        Store the entry as valid or warning depending on non-blocking findings\;
    }
    \caption{Cashflow validation routine}
    \label{alg:cashflow_validation}
\end{algorithm}

The important point is not only that validation exists. It is that validation results are persisted. This allows the frontend to display row-level feedback, summaries, and rejection reports instead of losing the reason behind a failed invoice.
